    ...
    if (context.group_id) {
        ...
        if (/测试番剧搜索:(.*)/.exec(context.message)) {
            if(state && state.enable){
                replyMsg(context, '群里已经有人在使用夜夜酱搜索番剧啦！', true);
                return true;
            }
        ...
        if (/确认搜索(\d?)/.exec(context.message)) {
            if(state.enable && state.nowstep == 2){
                if(state.useuser == user){
                    ...
                    replyMsg(context, `夜夜酱已全力在磁力网络中寻找《${chooseBangumi.name}》，请主人稍等片刻...`);
                    ...
                    searchAnimeStep2(chooseBangumi).then((step2result) => {
                        if(step2result.success){
                            ...
                            logger.setSearchAnimeStep(group,3);
                            logger.setSearchSourceList(group,subtitleGroupList,sourcelist,dateandep);
                            replyMsg(context, msg);
                        }else{
                            msg = '夜夜酱好像出现了点问题呢，请主人查看：\n' + step2result.err;
                            replyMsg(context, msg);
